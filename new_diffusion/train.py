...
def get_weight_dtype(dtype_name: str = "fp32") -> torch.dtype:
    try:
        return TRAIN_DTYPES[dtype_name]
    except KeyError as exc:
        raise ValueError(f"Unsupported --train_dtype={dtype_name!r}; choose fp32 or bf16.") from exc
...
def autocast_context(device: torch.device, dtype: torch.dtype):
    ...
    raise ValueError("BF16 training requires CUDA or CPU autocast support.")
...
def load_autoencoder(args: argparse.Namespace, device: torch.device) -> nn.Module:
    return DiffusersAutoencoder(scaling_factor=args.latent_scaling_factor).to(device)
...
def checkpoint_model_state(checkpoint) -> tuple[Mapping[str, torch.Tensor], bool]:
    """Extract a UNet state dict from full, wrapped, or plain checkpoints."""
    if not isinstance(checkpoint, Mapping):
        raise TypeError("--resume checkpoint must be a mapping containing UNet weights.")
...
def main() -> None:
    ...
    parser.add_argument(
        "--train_dtype",
        choices=tuple(TRAIN_DTYPES),
        default="fp32",
        help="Compute precision: fp32 (default) or bf16 AMP. Model and optimizer weights remain FP32.",
    )
    ...
    compute_dtype = get_weight_dtype(args.train_dtype)
    if args.train_dtype == "bf16" and device.type == "cuda":
        bf16_supported = getattr(torch.cuda, "is_bf16_supported", lambda: False)()
        if not bf16_supported:
            raise RuntimeError("--train_dtype bf16 requires a CUDA GPU with BF16 support.")
    out_dir = ensure_dir(args.checkpoint_dir)

    # Keep trainable parameters and optimizer state in FP32. BF16 is used only
    # by autocast for eligible matrix/convolution operations.
    ...
    if is_main_process():
        print(
            ...
            f"parameter_dtype={weight_dtype}"
        )

    model = build_diffusion_model(args).to(device=device, dtype=weight_dtype)
    if is_distributed():
    ...
    optim = AdamW(trainable_parameters, lr=args.lr, weight_decay=1e-4)
    ...
    if args.resume:
        ...
        model_state, is_wrapped_checkpoint = checkpoint_model_state(ckpt)
        unwrap_model(model).load_state_dict(model_state, strict=True)
        if is_wrapped_checkpoint and "optimizer" in ckpt:
            try:
                optim.load_state_dict(ckpt["optimizer"])
            except ValueError as exc:
                if is_main_process():
                    print(f"Warning: optimizer state was not restored ({exc}); continuing with a fresh optimizer.")
        if is_wrapped_checkpoint and "scaler" in ckpt:
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in pbar:
            ...
            target = batch["target_image"].to(device=device, dtype=weight_dtype, non_blocking=True)
            source = batch["source_image"].to(device=device, dtype=weight_dtype, non_blocking=True)
            pose = batch["target_pose"].to(device=device, dtype=weight_dtype, non_blocking=True)

            with torch.no_grad(), autocast_context(device, compute_dtype):
                target_latent = autoencoder.encode(
                    target.to(dtype=weight_dtype), sample_posterior=False
                ).latent
                source_latent = autoencoder.encode(
                    source.to(dtype=weight_dtype), sample_posterior=False
                ).latent
