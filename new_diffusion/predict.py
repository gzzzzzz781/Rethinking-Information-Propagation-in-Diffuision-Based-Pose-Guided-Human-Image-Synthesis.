...
def autocast_context(device: torch.device, dtype: torch.dtype):
    ...
    raise ValueError(f"Floating-point autocast is unavailable on device {device}.")
...
def apply_checkpoint_config(args: argparse.Namespace, checkpoint: Mapping) -> dict[str, object]:
    """Restore architecture and diffusion settings required by a checkpoint."""
    if not getattr(args, "checkpoint_config", True):
        return {}
...
def build_models(args: argparse.Namespace, device: torch.device) -> tuple[nn.Module, nn.Module, GaussianDiffusion]:
    ...
    applied_config = apply_checkpoint_config(args, ckpt)
    dtype_name = getattr(args, "dtype", "fp32")
    compute_dtype = get_inference_dtype(dtype_name, device)
    # Match train.py's AMP semantics: keep model/VAE parameters in FP32 and
    # use the selected dtype only for eligible forward operations.
    ...
    args.compute_dtype = compute_dtype
    ...
    if is_main_process() and applied_config:
        print(
            "Checkpoint config: "
            f"base_channels={args.base_channels}, latent_channels={args.latent_channels}, "
            f"timesteps={args.timesteps}, schedule={args.schedule}, "
            f"objective={args.diffusion_objective}"
        )
...
def sample_from_inputs(
    ...
    if compute_dtype is None:
        compute_dtype = get_inference_dtype(getattr(args, "dtype", "fp32"), device)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=True,
        help="Restore UNet architecture and diffusion settings from the checkpoint (default: enabled).",
    )
    ...
    parser.add_argument(
        ...
        default="fp32",
        help="Inference compute precision: fp32 or bf16 AMP. Model/VAE parameters remain FP32.",
    )
