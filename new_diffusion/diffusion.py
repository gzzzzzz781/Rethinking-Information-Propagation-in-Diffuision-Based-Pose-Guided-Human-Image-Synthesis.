...
class GaussianDiffusion(nn.Module):
    ...
    def _apply_cond_drop(self, cond, source_mask: torch.Tensor | None = None, pose_mask: torch.Tensor | None = None):
        def apply_mask(value: torch.Tensor, mask: torch.Tensor | None) -> torch.Tensor:
            if mask is None:
                return value
            # Preserve the condition dtype (important for BF16 training) when
            # applying classifier-free guidance masks created as FP32 scalars.
    ...
    def training_step(
        ...
        # Keep the objective and reductions in FP32 under BF16 autocast. This
        # preserves small gradients while the UNet forward still uses BF16
        # Tensor Core kernels.
        loss_map = (model_output.float() - target.float()) ** 2
